...
| 字段              | 类型        | 说明                                                    |
| ----------------- | ----------- | ------------------------------------------------------- |
| `personPresent`   | boolean     | 是否识别到人。若为 `false`，应用应使用默认推荐          |
| `heightCm`        | number      | 身高厘米，合理范围 80–240                               |
| `shoulderWidthCm` | number      | 肩宽厘米，可选                                          |
| `ageRange`        | enum/string | `child` / `teen` / `adult` / `senior` / `unknown`       |
| `gender`          | enum/string | `male` / `female` / `unknown`；只用于推荐，不做身份识别 |
| `bodyType`        | enum/string | `slim` / `regular` / `strong` / `unknown`               |
| `upperColor`      | string      | 上衣主色简述，可选                                      |
