# Windows Default Audio Adapter Evidence

`capture-default-audio` is an external VM Host Adapter operation. It proves
that the daemon can complete its fixed non-silent calibration through the active
Windows default output while a synchronized PCM loopback capture is
...
The production adapter must begin default-output PCM capture before calling
...
raw daemon response to bind the requested challenge, a live-format
...
session and Windows-default-output state. Both artifacts use immutable
...
The client rejects browser or Tauri cue substitution, missing or inactive
Windows default output, missing raw daemon response, stale kiosk session
binding, challenge mismatch, malformed evidence, digest mismatch, silent
