...
describe("repository script inventory guard", () => {
  ...
  it("rejects retired maintenance architecture from accepted workflows and scripts", () => {
    withFixture(
      {
        ...
        "scripts/testbed/kiosk-boundary.ps1": "Write-Host boundary",
        "scripts/windows/managed-route.ps1": "Write-Host managed",
        "scripts/windows/smoke.ps1": "Write-Host smoke",
        "scripts/testbed/workflows.mjs": "const ok = true;",
      },
      (root) => {
        const result = checkRepositoryScriptInventory({
          root,
          inventory: [
            ...
            {
              path: "scripts/testbed/kiosk-boundary.ps1",
              owner: "field-operations",
              category: "operator operation",
              workflows: ["kiosk lockdown"],
            },
            {
              path: "scripts/windows/managed-route.ps1",
              owner: "field-operations",
              category: "verifier-test guard",
            ...
            {
              path: "scripts/testbed/workflows.mjs",
              owner: "field-operations",
              category: "canonical entrypoint",
        ...
        for (const label of [
          ...
          assert.ok(
            result.failures
              .join("\n")
              .includes(`retired maintenance architecture (${label})`),
          );
        }
      },
    );
