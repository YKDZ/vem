...
test("packages a field kit with verified members, zip, and fixed-size parts", () => {
  ...
  const result = packageFieldKit(args) as {
...
test("reuses an unchanged field kit output and rebuilds with --force", () => {
  const root = mkdtempSync(join(tmpdir(), "vem-field-kit-cache-test-"));
  roots.push(root);
  const source = join(root, "source");
  const out = join(root, "out");
  ...
  const first = packageFieldKit(args) as {
    zip: { name: string; byteSize: number; sha256: string };
    parts: { name: string; byteSize: number; sha256: string }[];
    members: { name: string; byteSize: number; sha256: string }[];
    outDir: string;
  };
  assert.ok(existsSync(join(out, "vem-field-kit.cache.json")));
  const second = packageFieldKit(args) as typeof first;
  assert.equal(second.zip.sha256, first.zip.sha256);
  assert.equal(second.zip.byteSize, first.zip.byteSize);
  assert.deepEqual(second.parts, first.parts);
  assert.deepEqual(second.members, first.members);
  const forced = packageFieldKit([...args, "--force"]) as typeof first;
  assert.equal(forced.zip.sha256, first.zip.sha256);
  assert.ok(existsSync(join(out, "vem-field-kit.cache.json")));
});
