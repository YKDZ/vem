...
function projectFactoryRuntimeBoundary(profile) {
  ...
  const result = spawnSync(
    ...
    [
      ...
      "C:\\input\\MicrosoftEdgeWebView2RuntimeInstallerX64.exe",
...
`,
  ...
  it("accepts an unclaimed factory machine config but rejects retained identity or credentials", () => {
    ...
    assert.doesNotMatch(script, /C:\\ProgramData\\VEM\\secrets/);
    assert.doesNotMatch(script, /protectedSecretFileCount/);
...
`,
  ...
  it("plans VM runtime acceptance from an approved preclaim base", () => {
    ...
    try {
      ...
      assert.equal(
        plan.artifacts.simulatedHardwareSaleFlow,
        "artifacts/vm-runtime-acceptance/RUN-181/simulated-hardware-sale-flow-response.json",
      );
      assert.equal(
        plan.artifacts.serialConformance,
        "artifacts/vm-runtime-acceptance/RUN-181/serial-com-scanner-sale-conformance.json",
      );
      assert.deepEqual(
