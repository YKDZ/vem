...
import { collectInstalledAiDegradationEvidence } from "./ai-installed-degradation.ts";
import {
...
} from "./ai-virtual-try-on-installed-entry.ts";
...
const installedEntry = join(
  repoRoot,
  "scripts/testbed/ai-virtual-try-on-installed-entry.ts",
);
...
test("resolves the generated Vision manifest from a Windows module file URL", () => {
  const moduleUrl =
    "file:///C:/VEM/source/scripts/testbed/ai-virtual-try-on-installed-entry.ts";
  const mistakenPath = pathToFileURL(moduleUrl).pathname;
  assert.notEqual(
    mistakenPath,
    "C:\\VEM\\source\\scripts\\testbed\\ai-virtual-try-on-installed-entry.ts",
  );
...
test("proves missing model degradation from product detail without catalog-page evidence", async () => {
  assert.doesNotMatch(
    readFileSync(
      join(repoRoot, "scripts/testbed/ai-installed-degradation.ts"),
      "utf8",
    ),
    /catalog-page/,
  );
...
test("AI installed attempts wait for the same 10-minute product terminal window", () => {
  const timeouts = readFileSync(
    new URL("./try-on-timeouts.ts", import.meta.url),
    "utf8",
  );
  const entry = readFileSync(installedEntry, "utf8");
...
test("archives held regional evidence without overwriting an existing attempt member", () => {
  const source = readFileSync(
    join(repoRoot, "scripts/testbed/ai-virtual-try-on-installed-entry.ts"),
    "utf8",
  );
