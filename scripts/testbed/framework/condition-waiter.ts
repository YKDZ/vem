...
export async function waitForCondition<T = unknown>(
  name: string,
  predicate: () => ConditionResult<T> | Promise<ConditionResult<T>>,
  {
    timeoutMs = 60_000,
    pollMs = 250,
    signal = undefined,
  ...
  while (Date.now() < deadline) {
    ...
    if (lastObservation?.ok) return lastObservation.value as T;
 ...
 */
export function condition(ok: unknown, value: unknown = null): ConditionResult {
