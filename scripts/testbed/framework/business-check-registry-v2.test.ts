...
} from "./business-check-registry-v2.ts";
  ...
  return createBusinessCheckRegistryV2([
    {
      name: "commissioning",
      fullRequired: true,
      runner: { kind: "node", script: "commissioning.ts" },
    ...
    },
    {
      ...
      core: true,
      fullRequired: true,
      runner: { kind: "node", script: "sale.ts" },
      validator: (set) => ({
...
describe("business check registry v2", () => {
  ...
  it("attaches supporting evidence without changing the business status", () => {
    ...
    assert.deepEqual(
      evidence.supportingEvidence.map(
        (entry) => (entry as { kind: string }).kind,
      ),
      ["screenshot", "log"],
    );
  });
