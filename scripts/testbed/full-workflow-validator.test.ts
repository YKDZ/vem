...
describe("full workflow aggregate validator", () => {
  ...
  it("accepts the functional execution marker only when strict", () => {
    const original = process.env.VEM_VM_ACCEPTANCE_SKIP_AI_RSS;
    try {
      process.env.VEM_VM_ACCEPTANCE_SKIP_AI_RSS = "1";
      const report = aiVirtualTryOnReport();
      ...
      const invalid = aiVirtualTryOnReport();
      invalid.execution.aiRssObservation = "skipped_by_vm_acceptance";
      invalid.execution.functional = false;
      for (const attempt of invalid.attempts)
        delete attempt.result.peakRssBytes;
      assert.equal(
        validateBusinessCheckReport(
          descriptor("aiVirtualTryOn"),
