...
} from "./build-win10-baseline.ts";
import {
  createRuntimeProfile,
  renderLibvirtDomainXml,
} from "./libvirt-runtime-profile.ts";
import {
...
import {
  publishVerifiedBaselineRelease,
  runtimeProfileForPublishedRelease,
} from ${JSON.stringify(new URL("./linux-kvm-baseline.ts", import.meta.url).href)};
...
function runPublicationKillChild(root, configPath, faultStage, statePath) {
  const childPath = join(root, "kvm-baseline-publication-kill-child.ts");
...
import { runWithConstructionSignalCleanup } from ${JSON.stringify(
    new URL("./build-win10-baseline.ts", import.meta.url).href,
  )};
...
function trackedConstructionSignalCleanupChildSource() {
  return `
const baseline = await import(${JSON.stringify(new URL("./build-win10-baseline.ts", import.meta.url).href)});
const {
  constructionCleanup,
  createConstructionCommandTracker,
  runWithConstructionSignalCleanup,
...
const builder = await import(${JSON.stringify(new URL("./build-win10-baseline.ts", import.meta.url).href)});
const linux = await import(${JSON.stringify(new URL("./linux-kvm-baseline.ts", import.meta.url).href)});
...
describe("Linux KVM Windows baseline", () => {
  ...
  it("defines the portable portrait runtime profile without host-specific defaults", () => {
    ...
    assert.match(xml, /<clock offset="timezone" timezone="Asia\/Shanghai"\/>/);
    assert.match(
      xml,
  ...
  it(
    ...
    async () => {
      ...
      const xvfbPath = join(root, "fake-xvfb.ts");
      const windowManagerPath = join(root, "fake-window-manager.ts");
      const viewerPath = join(root, "fake-gvncviewer.ts");
  ...
  it(
    ...
    async () => {
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        const xvfbPath = join(root, "fake-xvfb.ts");
        const gatekeeperPath = join(root, "fake-gatekeeper.ts");
...
`,
  ...
  it("has no SPICE or QXL installer contract, media, state, or cleanup plumbing", () => {
    ...
    for (const file of [
      "build-win10-baseline.ts",
      "linux-kvm-baseline.ts",
      "prepare-vm-runtime.ps1",
      "verify-vm-runtime.ps1",
      "libvirt-runtime-profile.ts",
    ]) {
      assert.doesNotMatch(
  ...
  it("distinguishes signed viogpudo releases with the same INF basename by package content", async () => {
    ...
    try {
      ...
      const { createVirtioGpuDriverPackageIdentity } =
        await import("./build-win10-baseline.ts");
  ...
  it("keeps guest preparation separate from VM-only runner and toolchain setup", () => {
    ...
    const builder = readFileSync(
      new URL("./build-win10-baseline.ts", root),
      "utf8",
    );
    ...
    assert.match(shared, /sc\.exe stop/);
    assert.match(shared, /TimeoutSeconds = 20/);
  ...
  it("cleans the exact construction domain and both staging paths when its process receives SIGTERM", () => {
    ...
    const childPath = join(root, "construction-sigterm-child.ts");
  ...
  it("terminates in-flight execFile children before cleanup and blocks a late domain creator", () => {
    ...
    const childPath = join(root, "construction-tracked-sigterm-child.ts");
    const fakeVirshPath = join(root, "bin", "virsh");
    const longChildPath = join(root, "long-lived-command.ts");
    const lateDomainCreatorPath = join(root, "late-domain-creator.ts");
...
`,
  ...
  it(
    ...
    async () => {
      ...
      const configPath = join(root, "config.json");
      const childPath = join(root, "hard-crash-child.ts");
      const xvfbPath = join(root, "fake-xvfb.ts");
      const windowManagerPath = join(root, "fake-window-manager.ts");
      const viewerPath = join(root, "fake-viewer.ts");
...
`,
  ...
  it(
    ...
    async () => {
      ...
      const configPath = join(root, "config.json");
      const childPath = join(root, "launch-window-child.ts");
      const xvfbPath = join(root, "fake-xvfb.ts");
      const windowManagerPath = join(root, "fake-window-manager.ts");
      const viewerPath = join(root, "fake-viewer.ts");
...
`,
  ...
  it("preserves an unowned cache staging collision while allocating a new build identity", async () => {
    ...
    try {
      ...
      const baseline = await import("./build-win10-baseline.ts");
