...
try {
  ...
  Set-Content -LiteralPath (Join-Path $stale ".vem-ai-acceptance-owner.json") -Value '{}'
  & $module { param($Owned) Remove-TestbedAiAcceptanceArtifactRoot -Root $Owned -RunId "new-run" -FixtureKey "aiVirtualTryOn" } $stale
  if (Test-Path -LiteralPath $stale) { throw "stale AI artifact root was retained" }
