...
export function validateFactoryMaintenanceRelayAttestation(value) {
  ...
  exactKeys(
    session.relay,
    ["id", "role", "publicKey", "tunnelAddress", "endpoint"],
    "session.relay",
  );
  ...
  const relayAddress = ipv4(
    session.relay.tunnelAddress,
    "session.relay.tunnelAddress",
  );
  const relayEndpoint = endpoint(
    session.relay.endpoint,
    "session.relay.endpoint",
  );
  ...
  if (
    key(runner.relayPeer.publicKey, "runner.relayPeer.publicKey") !==
      relayPublicKey ||
    endpoint(runner.relayPeer.endpoint, "runner.relayPeer.endpoint") !==
      relayEndpoint
  ) {
    throw new Error(
  ...
  const relayCidr = `${relayAddress}/32`;
  if (
    !Array.isArray(runner.relayPeer.allowedIps) ||
    runner.relayPeer.allowedIps.length !== 1 ||
    runner.relayPeer.allowedIps[0] !== relayCidr
  ) {
    throw new Error(
      "runner relay peer must have exactly the session relay /32 AllowedIPs",
    );
  }
  ...
  if (
    runner.route.destination !== relayCidr ||
    runner.route.device !== runner.interface ||
    runner.route.source !== sourceAddress
  ) {
    throw new Error(
      "runner route does not bind the session relay /32, interface, and source address",
    );
  }
  return value;
}
...
export function collectFactoryMaintenanceRelayAttestation(session) {
  ...
  const route = parseRoute(
    run("ip", ["route", "get", relay.tunnelAddress]),
    relay.tunnelAddress,
  );
