...
} from "./hardware-lifecycle-guest-full.ts";
import {
  buildMqttTopic,
...
} from "./host-serial-control-plane.ts";
import { writePaymentMockCreateGateState } from "./mock-payment-create-gate.ts";
import {
...
const adapterPath = new URL(
  "./qemu-usb-serial-host-adapter.ts",
  import.meta.url,
).pathname;
...
describe("host serial control plane", () => {
  ...
  it("uses the repo-owned adapter wrapper and local mosquitto topic capture", () => {
    const implementation = readFileSync(
      new URL("./host-serial-control-plane.ts", import.meta.url),
      "utf8",
    );
    assert.match(implementation, /run-vm-host-adapter\.ts/);
  ...
  it("selects lifecycle devices by target port after libvirt normalizes aliases", () => {
    const domainXml = `<domain><devices>
      <serial type='pty'><source path='/dev/pts/41'/><target type='usb-serial' port='0'><model name='usb-serial'/></target><alias name='serial0'/><address type='usb' bus='0' port='3.1'/></serial>
      <serial type='pty'><source path='/dev/pts/42'/><target type='usb-serial' port='1'><model name='usb-serial'/></target><alias name='serial1'/><address type='usb' bus='0' port='3.2'/></serial>
    </devices></domain>`;

    assert.match(
      serialDeviceXmlForRole(domainXml, "lower-controller"),
      /port='0'/,
    );
    ...
    assert.throws(
      () =>
        serialDeviceXmlForRole(
          domainXml.replace("port='1'", "port='2'"),
          "scanner",
        ),
      /target port 1/,
    );
  ...
  it("enforces a deadline and escalates a SIGTERM-resistant host command to SIGKILL", async () => {
    ...
    const childPath = join(root, "resistant-child.ts");
    writeFileSync(
      childPath,
