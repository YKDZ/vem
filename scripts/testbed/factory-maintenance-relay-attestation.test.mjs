...
describe("Factory maintenance relay attestation", () => {
  ...
  it("collects the relay route as the session relay /32 from runner-owned observations", () => {
    ...
    try {
      writeFileSync(
        join(root, "sudo"),
        `#!/bin/sh\ncase "$*" in\n  "wg show wg-factory dump") printf '%s\\n' '${relayKey}\t(none)\trelay.example.test:51820\t10.91.0.1/32\t${handshake}\t0\t0\t0' ;;\n  "wg show wg-factory latest-handshakes") printf '%s\\n' '${relayKey}\t${handshake}' ;;\n  "wg show wg-factory allowed-ips") printf '%s\\n' '${relayKey}\t10.91.0.1/32' ;;\n  *) exit 1 ;;\nesac\n`,
        { mode: 0o755 },
      );
      writeFileSync(
        join(root, "ip"),
        "#!/bin/sh\nprintf '%s\\n' '10.91.0.1 dev wg-factory src 10.91.2.10 uid 1000'\n",
        { mode: 0o755 },
      );
