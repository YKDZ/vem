...
`;
  ...
  boundedIt(
    ...
    () => {
      ...
      assert.match(
        behavior,
        /\$previousUnconfirmedPreDisarmOperationFailure = \[Environment\]::GetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \[EnvironmentVariableTarget\]::Process\)[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", "1", \[EnvironmentVariableTarget\]::Process\)[\s\S]*?behavior\.watchdog-missing-completion-unconfirmed[\s\S]*?HarnessDeadlineUtc \(\[DateTime\]::UtcNow\.AddSeconds\(8\)\)[\s\S]*?could not terminate process \[0-9\]\+: missing-completion[\s\S]*?\$primaryFailureStage = "behavior\.primary-job-unavailable"[\s\S]*?\$primaryFailureStageRoot = Join-Path \$root \("diagnostics\\" \+ \$primaryFailureStage \+ "-" \+ \("0" \* 32\)\)[\s\S]*?\$primaryFailureWatchdogRoot = Join-Path \(Join-Path \$primaryFailureStageRoot "suspended-process-watchdog"\) \("0" \* 32\)[\s\S]*?Assert-True \(\$primaryFailureStageRoot\.Length -lt 248\)[\s\S]*?Assert-True \(\$primaryFailureWatchdogRoot\.Length -lt 248\)[\s\S]*?Invoke-BoundedPowerShell -Stage \$primaryFailureStage[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \$previousUnconfirmedPreDisarmOperationFailure/,
      );
      assert.match(
        behavior,
