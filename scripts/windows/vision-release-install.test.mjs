...
`;
  ...
  boundedIt(
    ...
    () => {
      ...
      assert.match(
        behavior,
        /\$previousUnconfirmedPreDisarmOperationFailure = \[Environment\]::GetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \[EnvironmentVariableTarget\]::Process\)[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", "1", \[EnvironmentVariableTarget\]::Process\)[\s\S]*?behavior\.watchdog-missing-completion-unconfirmed[\s\S]*?HarnessDeadlineUtc \(\[DateTime\]::UtcNow\.AddSeconds\(12\)\)[\s\S]*?fixture forced pre-disarm operation failure[\s\S]*?could not terminate process \[0-9\]\+: missing-completion[\s\S]*?status=suspended-process-watchdog-armed detail=processId=\[0-9\]\+ identity=original-process-handle[\s\S]*?status=process-ownership detail=state=job-assigned-suspended processId=\[0-9\]\+[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \$null, \[EnvironmentVariableTarget\]::Process\)[\s\S]*?\$primaryFailureStage = "behavior\.primary-job-unavailable"[\s\S]*?\$primaryFailureStageRoot = Join-Path \$root \("diagnostics\\" \+ \$primaryFailureStage \+ "-" \+ \("0" \* 32\)\)[\s\S]*?\$primaryFailureWatchdogRoot = Join-Path \(Join-Path \$primaryFailureStageRoot "suspended-process-watchdog"\) \("0" \* 32\)[\s\S]*?Assert-True \(\$primaryFailureStageRoot\.Length -lt 248\)[\s\S]*?Assert-True \(\$primaryFailureWatchdogRoot\.Length -lt 248\)[\s\S]*?\$primaryFailureRecords = New-Object 'System\.Collections\.Generic\.List\[object\]'[\s\S]*?Invoke-BoundedPowerShell -Stage \$primaryFailureStage[\s\S]*?\$primaryFailure\.InnerExceptions\[0\]\.Message -match "failed with exit code 23"[\s\S]*?status=process-ownership detail=state=resumed-job-assigned processId=\[0-9\]\+[\s\S]*?status=failed detail=exitCode=23[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \$previousUnconfirmedPreDisarmOperationFailure/,
      );
      assert.match(
        behavior,
