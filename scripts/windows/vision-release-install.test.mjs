...
`;
  ...
  boundedIt(
    ...
    () => {
      ...
      assert.match(
        behavior,
        /function Assert-HarnessWatchdogStagePathBudget\(\[string\]\$Stage\) \{[\s\S]*?\$stageRoot = Join-Path \$root \("diagnostics\\" \+ \$Stage \+ "-" \+ \("0" \* 32\)\)[\s\S]*?\$watchdogRoot = Join-Path \(Join-Path \$stageRoot "suspended-process-watchdog"\) \("0" \* 32\)[\s\S]*?\$fixtureDeadlinePath = Join-Path \$watchdogRoot "ready\.deadline"[\s\S]*?\$fixtureConfirmationPath = Join-Path \$watchdogRoot "ready\.confirm"[\s\S]*?Assert-True \(\$stageRoot\.Length -le 220\)[\s\S]*?Assert-True \(\$watchdogRoot\.Length -le 220\)[\s\S]*?Assert-True \(\$fixtureDeadlinePath\.Length -le 220\)[\s\S]*?Assert-True \(\$fixtureConfirmationPath\.Length -le 220\)[\s\S]*?\$delayedDisarmStage = "b\.dh"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$delayedDisarmStage[\s\S]*?\$commandWriteFailureStage = "b\.wf"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$commandWriteFailureStage[\s\S]*?Invoke-BoundedPowerShell -Stage \$commandWriteFailureStage[\s\S]*?\$unconfirmedStage = "b\.wu"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$unconfirmedStage[\s\S]*?\$primaryFailureStage = "b\.pj"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$primaryFailureStage[\s\S]*?\$setupTimeoutStage = "b\.st"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$setupTimeoutStage/,
      );
      assert.match(
        behavior,
