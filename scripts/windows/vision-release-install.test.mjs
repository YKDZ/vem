...
`;
  ...
  boundedIt(
    ...
    () => {
      ...
      assert.match(
        behavior,
        /function Assert-HarnessWatchdogStagePathBudget\(\[string\]\$Stage\) \{[\s\S]*?\$stageRoot = Join-Path \$root \("diagnostics\\" \+ \$Stage \+ "-" \+ \("0" \* 32\)\)[\s\S]*?\$watchdogRoot = Join-Path \(Join-Path \$stageRoot "suspended-process-watchdog"\) \("0" \* 32\)[\s\S]*?Assert-True \(\$stageRoot\.Length -lt 248\)[\s\S]*?Assert-True \(\$watchdogRoot\.Length -lt 248\)[\s\S]*?\$commandWriteFailureStage = "behavior\.watchdog-command-write-failure"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$commandWriteFailureStage[\s\S]*?Invoke-BoundedPowerShell -Stage \$commandWriteFailureStage[\s\S]*?\$unconfirmedStage = "behavior\.watchdog-unconfirmed"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$unconfirmedStage[\s\S]*?\$primaryFailureStage = "behavior\.primary-job-unavailable"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$primaryFailureStage/,
      );
      assert.match(
        behavior,
