...
`;
  ...
  boundedIt(
    ...
    () => {
      ...
      assert.match(
        behavior,
        /VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE[\s\S]*?VEM_VISION_HARNESS_FIXTURE_FORCE_WATCHDOG_DISARM_COMMAND_WRITE_FAILURE[\s\S]*?"1"[\s\S]*?behavior\.watchdog-write-failure[\s\S]*?fixture forced pre-disarm operation failure[\s\S]*?fixture forced watchdog disarm command write failure[\s\S]*?suspended-process-termination-confirmed[\s\S]*?suspended-process-watchdog-closed[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \$previousPreDisarmOperationFailure[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_WATCHDOG_DISARM_COMMAND_WRITE_FAILURE", \$previousWatchdogDisarmCommandWriteFailure/,
      );
      assert.doesNotMatch(
        harness,
      ...
      assert.match(
        behavior,
        /function Assert-HarnessWatchdogStagePathBudget\(\[string\]\$Stage\) \{[\s\S]*?\$stageRoot = Join-Path \$root \("diagnostics\\" \+ \$Stage \+ "-" \+ \("0" \* 32\)\)[\s\S]*?\$watchdogRoot = Join-Path \(Join-Path \$stageRoot "suspended-process-watchdog"\) \("0" \* 32\)[\s\S]*?\$fixtureDeadlinePath = Join-Path \$watchdogRoot "ready\.deadline"[\s\S]*?\$fixtureConfirmationPath = Join-Path \$watchdogRoot "ready\.confirm"[\s\S]*?Assert-True \(\$stageRoot\.Length -lt 248\)[\s\S]*?Assert-True \(\$watchdogRoot\.Length -lt 248\)[\s\S]*?Assert-True \(\$fixtureDeadlinePath\.Length -lt 248\)[\s\S]*?Assert-True \(\$fixtureConfirmationPath\.Length -lt 248\)[\s\S]*?\$commandWriteFailureStage = "behavior\.watchdog-write-failure"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$commandWriteFailureStage[\s\S]*?Invoke-BoundedPowerShell -Stage \$commandWriteFailureStage[\s\S]*?\$unconfirmedStage = "behavior\.watchdog-unconfirmed"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$unconfirmedStage[\s\S]*?\$primaryFailureStage = "behavior\.primary-job-unavailable"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$primaryFailureStage[\s\S]*?\$setupTimeoutStage = "behavior\.watchdog-setup-timeout"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$setupTimeoutStage/,
      );
      assert.match(
        behavior,
        /\$previousUnconfirmedPreDisarmOperationFailure = \[Environment\]::GetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \[EnvironmentVariableTarget\]::Process\)[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", "1", \[EnvironmentVariableTarget\]::Process\)[\s\S]*?\$unconfirmedStage = "behavior\.watchdog-unconfirmed"[\s\S]*?HarnessDeadlineUtc \(\[DateTime\]::UtcNow\.AddSeconds\(12\)\)[\s\S]*?fixture forced pre-disarm operation failure[\s\S]*?could not terminate process \[0-9\]\+: missing-completion[\s\S]*?stage=\$unconfirmedStage status=suspended-process-watchdog-armed detail=processId=\[0-9\]\+ identity=original-process-handle[\s\S]*?stage=\$unconfirmedStage status=process-ownership detail=state=job-assigned-suspended processId=\[0-9\]\+[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \$null, \[EnvironmentVariableTarget\]::Process\)[\s\S]*?\$primaryFailureStage = "behavior\.primary-job-unavailable"[\s\S]*?\$primaryFailureRecords = New-Object 'System\.Collections\.Generic\.List\[object\]'[\s\S]*?Invoke-BoundedPowerShell -Stage \$primaryFailureStage[\s\S]*?\$primaryFailure\.InnerExceptions\[0\]\.Message -match "failed with exit code 23"[\s\S]*?status=process-ownership detail=state=resumed-job-assigned processId=\[0-9\]\+[\s\S]*?status=failed detail=exitCode=23[\s\S]*?SetEnvironmentVariable\("VEM_VISION_HARNESS_FIXTURE_FORCE_PRE_DISARM_OPERATION_FAILURE", \$previousUnconfirmedPreDisarmOperationFailure/,
      );
      assert.match(
        behavior,
