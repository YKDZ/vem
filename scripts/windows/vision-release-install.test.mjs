...
`;
  ...
  boundedIt(
    ...
    () => {
      ...
      assert.match(
        behavior,
        /function Assert-HarnessWatchdogStagePathBudget\(\[string\]\$Stage\) \{[\s\S]*?\$stageRoot = Join-Path \$root \("diagnostics\\" \+ \$Stage \+ "-" \+ \("0" \* 32\)\)[\s\S]*?\$watchdogRoot = Join-Path \(Join-Path \$stageRoot "suspended-process-watchdog"\) \("0" \* 32\)[\s\S]*?\$fixtureDeadlinePath = Join-Path \$watchdogRoot "ready\.deadline"[\s\S]*?\$fixtureConfirmationPath = Join-Path \$watchdogRoot "ready\.confirm"[\s\S]*?Assert-True \(\$stageRoot\.Length -lt 248\)[\s\S]*?Assert-True \(\$watchdogRoot\.Length -lt 248\)[\s\S]*?Assert-True \(\$fixtureDeadlinePath\.Length -lt 248\)[\s\S]*?Assert-True \(\$fixtureConfirmationPath\.Length -lt 248\)[\s\S]*?\$commandWriteFailureStage = "behavior\.watchdog-command-write-failure"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$commandWriteFailureStage[\s\S]*?Invoke-BoundedPowerShell -Stage \$commandWriteFailureStage[\s\S]*?\$unconfirmedStage = "behavior\.watchdog-unconfirmed"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$unconfirmedStage[\s\S]*?\$primaryFailureStage = "behavior\.primary-job-unavailable"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$primaryFailureStage[\s\S]*?\$setupTimeoutStage = "behavior\.watchdog-setup-timeout"[\s\S]*?Assert-HarnessWatchdogStagePathBudget -Stage \$setupTimeoutStage/,
      );
      assert.match(
        behavior,
