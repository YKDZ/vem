...
$descriptorDigest = Get-Digest (Join-Path $context.delivery "descriptor.json")
...
$descriptorPath = Join-Path $factoryRoot "vision-release\descriptor.json"
$originalDescriptorBytes = [IO.File]::ReadAllBytes($descriptorPath)
$whitespaceMutatedDescriptorBytes = New-Object byte[] ($originalDescriptorBytes.Length + 1)
$whitespaceMutatedDescriptorBytes[0] = 0x0A
[Array]::Copy($originalDescriptorBytes, 0, $whitespaceMutatedDescriptorBytes, 1, $originalDescriptorBytes.Length)
[IO.File]::WriteAllBytes($descriptorPath, $whitespaceMutatedDescriptorBytes)
$descriptorBindingRejected = $false
try { & "C:\VEM\bringup\install-vision-release.ps1" -BundlePath (Join-Path $factoryRoot "vision-release\bundle.bin") -DescriptorPath $descriptorPath -AttestationPath (Join-Path $factoryRoot "vision-release\attestation.json") -SbomPath (Join-Path $factoryRoot "vision-release\sbom.json") -ProvenancePath (Join-Path $factoryRoot "vision-release\provenance.json") -ConformanceEvidencePath (Join-Path $factoryRoot "vision-release\conformance.json") -ApprovalPath (Join-Path $factoryRoot "vision-release\approval.json") -FactoryManifestPath (Join-Path $factoryRoot "vision-release\factory-manifest.json") -ConfigurationPath (Join-Path $stateRoot "config\fixture.json") -EvidencePath $evidencePath -TaskUser $env:USERNAME } catch { $descriptorBindingRejected = $true }
Assert-True $descriptorBindingRejected "Factory Manifest descriptor evidence did not bind exact signed bytes"
[IO.File]::WriteAllBytes($descriptorPath, $originalDescriptorBytes)
  ...
  $badDescriptorDigest = Get-Digest (Join-Path $factoryRoot "vision-release\descriptor.json")
  ...
  $orphanDescriptorDigest = Get-Digest (Join-Path $factoryRoot "vision-release\descriptor.json")
