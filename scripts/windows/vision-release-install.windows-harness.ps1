...
$originalBundle = Join-Path $context.root "approved-bundle.bin"
  ...
  Assert-True ($rollbackFailed -and $rollbackEvidence.rollbackAttempted -and $rollbackEvidence.rollbackOk) "failed activation did not roll back: $($rollbackEvidence | ConvertTo-Json -Compress)"
