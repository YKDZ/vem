...
describe("VendingService line-level fulfillment", () => {
  ...
  it("refuses normal command resolution for protected drill commands before side effects", async () => {
    const db = {
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
    ...
    await expect(
      service.resolveCommand("cmd-drill", {
        result: "not_dispensed",
        note: "normal endpoint must not resolve drill evidence",
      }),
    ).rejects.toThrow(
  ...
  it("skips protected drill commands during timeout processing", async () => {
    const db = {
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockResolvedValue([
            {
              id: "cmd-drill-timeout",
              commandNo: "DRILL-CMD-TIMEOUT",
              orderId: "order-drill",
              slotId: "slot-1",
              payloadJson: {
                kind: "protected_fulfillment_drill",
    ...
    const result = await service.markTimedOutCommands(
  ...
  it("ignores daemon dispense results for protected drill commands before side effects", async () => {
    const payload = {
      commandNo: "DRILL-CMD-RESULT",
      success: false,
      errorCode: "NO_DROP",
      message: "drill result must stay simulated",
      reportedAt: "2026-06-26T07:00:00.000Z",
    };
    const eventValues = vi.fn().mockReturnValue({
      onConflictDoNothing: vi.fn().mockReturnValue({
        returning: vi.fn().mockResolvedValue([{ id: "event-drill" }]),
      }),
    });
    const update = vi.fn();
    const db = {
      select: vi.fn().mockReturnValue({
        from: vi.fn().mockReturnValue({
          where: vi.fn().mockResolvedValue([{ id: "machine-1", code: "M001" }]),
        }),
      }),
    ...
    const service = new VendingService(
      ...
      { bindVendingService: vi.fn() } as never,
      {
        verifyFromTopic: vi.fn().mockResolvedValue({
          payload,
          messageId: "result:DRILL-CMD-RESULT",
        }),
      } as never,
      notificationsService as never,
      { releaseAffectedReservationForDispenseFailure } as never,
      {} as never,
      {} as never,
      maintenanceWorkOrdersService as never,
    );
